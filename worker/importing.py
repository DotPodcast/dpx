...
DOTPODCAST_DOMAIN = 'dotpodcast.co'

IMPORTERS = (
    'importers.rss.RssImporter',
    'importers.rss.RssAsHtmlImporter'
)
...
def begin_import(task, url):
    ...
    for item in items:
        task.progress(
            1,
            {
                '+entries': importer.get_item_data(feed, item)
            }
        )
